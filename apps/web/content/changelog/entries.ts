import type { ComponentType } from "react";
import Kocteau030Changelog from "./kocteau-0-3-0.mdx";
import Kocteau0315Changelog from "./kocteau-0-3-15.mdx";
import Kocteau0314Changelog from "./kocteau-0-3-14.mdx";
import Kocteau0313Changelog from "./kocteau-0-3-13.mdx";
import Kocteau0312Changelog from "./kocteau-0-3-12.mdx";
import Kocteau0311Changelog from "./kocteau-0-3-11.mdx";
import Kocteau0310Changelog from "./kocteau-0-3-10.mdx";
import Kocteau039Changelog from "./kocteau-0-3-9.mdx";
import Kocteau038Changelog from "./kocteau-0-3-8.mdx";
import Kocteau037Changelog from "./kocteau-0-3-7.mdx";
import Kocteau036Changelog from "./kocteau-0-3-6.mdx";
import Kocteau035Changelog from "./kocteau-0-3-5.mdx";
import Kocteau034Changelog from "./kocteau-0-3-4.mdx";
import Kocteau033Changelog from "./kocteau-0-3-3.mdx";
import Kocteau032Changelog from "./kocteau-0-3-2.mdx";
import Kocteau031Changelog from "./kocteau-0-3-1.mdx";

export type PublicChangelogEntry = {
  slug: string;
  title: string;
  date: string;
  version?: string;
  summary: string;
  draft?: boolean;
  Content: ComponentType;
};

export const publicChangelogEntries: PublicChangelogEntry[] = [
  {
    slug: "kocteau-0-3-1",
    title: "A steadier mobile session",
    date: "2026-05-24",
    version: "0.3.1",
    summary:
      "A public draft from the latest release, focused on mobile polish and the reading experience.",
    Content: Kocteau031Changelog,
  },
  {
    slug: "kocteau-0-3-2",
    title: "A cleaner review loop",
    date: "2026-05-27",
    version: "0.3.2",
    summary:
      "A public draft from the latest release, focused on reviews and discovery.",
    Content: Kocteau032Changelog,
  },
  {
    slug: "kocteau-0-3-3",
    title: "Kocteau 0.3.3",
    date: "2026-06-01",
    version: "0.3.3",
    summary:
      "A public draft from the latest Kocteau release.",
    Content: Kocteau033Changelog,
  },
  {
    slug: "kocteau-0-3-4",
    title: "Kocteau 0.3.4",
    date: "2026-06-02",
    version: "0.3.4",
    summary:
      "A public draft from the latest Kocteau release.",
    Content: Kocteau034Changelog,
  },
  {
    slug: "kocteau-0-3-5",
    title: "A steadier mobile session",
    date: "2026-06-03",
    version: "0.3.5",
    summary:
      "A public draft from the latest release, focused on mobile polish and the reading experience.",
    Content: Kocteau035Changelog,
  },
  {
    slug: "kocteau-0-3-6",
    title: "A cleaner review loop",
    date: "2026-06-04",
    version: "0.3.6",
    summary:
      "A public draft from the latest release, focused on reviews and discovery.",
    Content: Kocteau036Changelog,
  },
  {
    slug: "kocteau-0-3-7",
    title: "A cleaner review loop",
    date: "2026-06-09",
    version: "0.3.7",
    summary:
      "A public draft from the latest release, focused on reviews and discovery.",
    Content: Kocteau037Changelog,
  },
  {
    slug: "kocteau-0-3-8",
    title: "A cleaner review loop",
    date: "2026-06-20",
    version: "0.3.8",
    summary:
      "A public draft from the latest release, focused on reviews and discovery.",
    Content: Kocteau038Changelog,
  },
  {
    slug: "kocteau-0-3-9",
    title: "A cleaner review loop",
    date: "2026-07-28",
    version: "0.3.9",
    summary:
      "A public draft from the latest release, focused on reviews and discovery.",
    Content: Kocteau039Changelog,
  },
  {
    slug: "kocteau-0-3-10",
    title: "A cleaner review loop",
    date: "2026-08-01",
    version: "0.3.10",
    summary:
      "A public draft from the latest release, focused on reviews and discovery.",
    Content: Kocteau0310Changelog,
  },
  {
    slug: "kocteau-0-3-11",
    title: "Kocteau 0.3.11",
    date: "2026-08-04",
    version: "0.3.11",
    summary:
      "A public draft from the latest Kocteau release.",
    Content: Kocteau0311Changelog,
  },
  {
    slug: "kocteau-0-3-12",
    title: "A steadier mobile session",
    date: "2026-08-11",
    version: "0.3.12",
    summary:
      "A public draft from the latest release, focused on mobile polish and the reading experience.",
    Content: Kocteau0312Changelog,
  },
  {
    slug: "kocteau-0-3-13",
    title: "A cleaner review loop",
    date: "2026-08-18",
    version: "0.3.13",
    summary:
      "A public draft from the latest release, focused on reviews and discovery.",
    Content: Kocteau0313Changelog,
  },
  {
    slug: "kocteau-0-3-14",
    title: "A cleaner review loop",
    date: "2026-08-24",
    version: "0.3.14",
    summary:
      "A public draft from the latest release, focused on reviews and discovery.",
    Content: Kocteau0314Changelog,
  },
  {
    slug: "kocteau-0-3-15",
    title: "Kocteau 0.3.15",
    date: "2026-09-21",
    version: "0.3.15",
    summary:
      "A public draft from the latest Kocteau release.",
    Content: Kocteau0315Changelog,
  },
  // public-changelog-entry
  {
    slug: "kocteau-0-3-0",
    title: "A calmer first session",
    date: "2026-05-18",
    version: "0.3.0",
    summary:
      "Mobile polish, onboarding refinements, and a steadier reading surface for early Kocteau listeners.",
    Content: Kocteau030Changelog,
  },
];
