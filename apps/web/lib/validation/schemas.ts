...
export const createReviewSchema = z.object({
  ...
  title: z.string().trim().min(1, "Track title is required.").max(160, "Track title is too long."),
  artist_name: optionalTrimmedString(160),
  artist_provider_id: optionalTrimmedString(40),
  artist_picture_url: z.string().trim().url("Artist image URL must be valid.").nullable().optional().transform((value) => value ?? null),
  album_provider_id: optionalTrimmedString(40),
  album_title: optionalTrimmedString(160),
  album_cover_url: z.string().trim().url("Album cover URL must be valid.").nullable().optional().transform((value) => value ?? null),
  album_deezer_url: z.string().trim().url("Album URL must be valid.").nullable().optional().transform((value) => value ?? null),
  album_release_date: z.string().trim().regex(/^\d{4}-\d{2}-\d{2}$/, "Album release date must use YYYY-MM-DD.").nullable().optional().transform((value) => value ?? null),
  album_record_type: optionalTrimmedString(40),
  cover_url: z.string().trim().url("Cover URL must be valid.").nullable().optional().transform((value) => value ?? null),
  deezer_url: z.string().trim().url("Deezer URL must be valid.").nullable().optional().transform((value) => value ?? null),
